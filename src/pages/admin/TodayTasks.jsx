...
const AdminTodayTasks = () => {
  ...
  const DISPLAY_COLUMNS = ["col3", "col4", "col15", "col31"];
  ...
  const fetchTodayData = async () => {
    try {
      ...
      toast.success(`Fetched ${filteredItems.length} Today Tasks`, {
        duration: 3000,
        position: "top-right",
      });
    } catch (err) {
  ...
  const filteredPendingTasks = todayTasks.filter((item) => {
    const term = searchTerm.toLowerCase();
    const col3Val = String(item.col3 || "").toLowerCase();
    ...
    const matchescol3 = col3Filter
      ? col3Val.includes(col3Filter.toLowerCase())
      : true;
    ...
    return matchesSearchTerm && matchescol3 && matchesCol4;
  });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="text-slate-600 font-medium">Loading Today tasks...</p>
        </div>
      </div>
    );
  }
      ...
      {filteredPendingTasks.length > 0 ? (
        <div className="bg-white rounded-lg border shadow-sm p-6">
          ...
          <div className="h-[calc(100vh-280px)] overflow-hidden">
            <TodayTasksTable
              isCompact={true}
              filterTasks={filteredPendingTasks}
            />
          </div>
        </div>
      ) : (
